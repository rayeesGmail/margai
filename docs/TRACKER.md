...
Built on `d15-ncert-books`, five commits (58b9f3f → the docs commit). Server tests 747 → 790,
...
HOW TO RESUME — one founder-launched command, and then a reading:
  1. `MARGAI_AI_COHERE_API_KEY` sourced (never inline; docs/runbooks/ai-provider-keys.md — the
     `live` profile refuses to start without it). No `AWS_PROFILE` is needed: nothing touches S3.
  2. `ncert embed --book phy11-part1` under `AI_LIVE=1` and the `pipeline` profile. It embeds 894
     paragraphs, then runs the 15 queries and prints top-3 per query with hit@1 / hit@3 / MRR.
     **Estimated well under ₹5** at the pinned model's 0.12 USD/1M tokens — the cheapest paid run
     this book has had. It resumes, so an interruption costs only the uncommitted batch, and a
     re-run re-scores for the price of 15 query embeddings.
  3. **What to read in the report, in this order**: (a) do the five Hindi queries reach English
     paragraphs — that is the pin, and a failure there is the one finding that is cheap now and
     expensive after nine more books (§4.9's founder rider of 2026-09-12 says the swap happens
     while it is still free); (b) does q08 reach ch 6 §6.9; (c) the "which half actually fired"
     table, now that the text half can fire; (d) hit@1 and MRR as the number D17 re-runs against
     ~9,000 paragraphs and compares to.
  4. Three things the run can find wrong, none of which I changed unilaterally: **the pin**, **the
     chunk** (a bare paragraph like "where m_i is the mass…" carries little standalone meaning; the
     first lever is prefixing the chapter/section heading before embedding, which §6.4's wording
     makes a spec question rather than a tuning knob), and **the weighting** (`rrf-k` 60 and
     `similarity-floor` 0.30 are opening bids with no number anywhere in the plan).
Then: **bio11** (still on the early v2, and it carries D14's tick) and the eight remaining books.
  D15's ✅ is a coverage report per book — one book of ten — plus this run.
```
