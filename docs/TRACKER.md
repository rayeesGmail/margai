...
```
D15 · 2026-09-25 · the apparatus boundary moves inside its page — code done at ₹0, the re-extraction
...
HOW TO RESUME (supersedes 1/1b of 2026-09-24 below; 2–4 there stand):
  1. **Re-extract the 19 heading pages — the founder's run.** Two one-line commands, each a plain
     resume that calls exactly those pages (bio11 14, phy11-part1 5), ≈₹85–100 at the long-run rate:
     `ncert extract --book bio11 --lang en` then `--book phy11-part1`, on `pipeline,live,visionopus`
     (runbook §3). Read each report's **"pages the Summary starts on"** checklist first: every page
     should carry paragraphs, none of them Summary text or a numbered exercise.
  2. `ncert load` both books (₹0), then `ncert verify` free, then `--read-pages` (resumes into the
     heading pages and any row whose words the merge changed, ≈₹20), then `ncert embed` both books.
     Only then are the two books canonical.
  3. The eight remaining books — the fix is already in their path.
  4. **Push `d15-ncert-books`** (≈125 commits ahead of `main`).
