...
ALSO FIXED: the runbook's §3 and chapter-7 extract commands said `--spring.profiles.active=pipeline,live`,
  ...
  **`ncert extract` has no transcriber guard**, so the profile left off spends a book's budget on
  Haiku and says so nowhere but the ledger. Both commands corrected and the trap written down;
  **the guard itself is not built** and is the obvious next ₹0 safety commit.
HOW TO RESUME:
  ...
  3. **The ₹0 safety commit worth doing first**: give `ncert extract` the transcriber guard that
     `ncert verify --read-pages` already has. It is the cheapest hour on this list and it removes
     the one failure that costs a whole book.
...
SESSION CLOSED 2026-09-22, tree clean. **105 commits on `d15-ncert-books` are ahead of `main`**
  (07e56a3 2026-09-14 → e4d5310 today, 106 counting this correction); PR #16 was merged 2026-09-14 and `main`/`origin/main` both
  sit at that merge (451adb3), so every day of D15 since is unreviewed and unpushed, and
  `origin/d15-ncert-books` is a stale ref at d0cfc39. **Pushing is now overdue** — the review grows
  with every book. Server tests 827, `./mvnw verify` green. **D15 is not ticked**: its ✅ is a
  coverage report per book and this is **two books of ten**. **Spend this session: ₹914.00.**
```
