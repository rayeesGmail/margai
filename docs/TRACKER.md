...
**Founder rulings on the smoke report, 2026-09-12** (this thread closes with them):
  ...
  **append-only correction note** — every embedding `cost_paise` written to date was priced off the
  ...
  rewrites history).
Founder-pending: ~~the Anthropic workspace spend limit + alert~~ **done 2026-09-12**. Still open —
  the **embed price**, which the founder could not find on the provider's pricing page and which I
  could not retrieve either (WebFetch is denied to sessions; a web search returned the pricing page
  but not the figure). **It blocks nothing, and here is why**: `cost_paise` rounds up (§4.8), so any
  embedding call under roughly 750–1,100 tokens — the exact crossover depends on the true rate —
  costs 1 paisa whatever the price actually is. Tonight's rows (7 and 27 tokens) would read 1 paisa
  at any rate between zero and ~1.5/1M, so the placeholder is invisible at this scale and only
  starts to distort when whole paragraphs are embedded in bulk. **The authoritative settlement is
  D16**: the corpus embedding is the first material spend, so the provider's invoice line divided by
  that day's summed `input_tokens` from `ai_calls` gives the exact $/1M from our own ledger, and it
  is the natural moment for §4.8's append-only correction note. Until then the 0.12 placeholder
  stands, recorded as a placeholder. **At D23** — re-run the milestone economics against the eval's
  first honest per-doubt cost (ruling 3).
Open after this: only the **direct-API embed price**, settled at D16 from the first real invoice
  against the ledger's token sum (above). Everything else in the switch is closed.
  Bedrock's revival and the residency question stay PARKED.
```
