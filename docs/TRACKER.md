...
Open after this: the **workspace spend limit and alert** (the independent backstop, §10.4) and the
  **exact embed model id and direct-API price** — the price row is still the Bedrock placeholder and
  every `cost_paise` is computed from it. Then the live acceptance, which is now runnable:
  `cd server && AI_LIVE=1 ./mvnw test -Dtest=AiLiveSmokeTest -Dsurefire.failIfNoSpecifiedTests=false`
  (founder-run: Claude sessions cannot read the env file or launch a live provider by design).
