...
Founder-pending: ~~the Anthropic workspace spend limit + alert~~ and ~~the embed price~~ — **both
  ...
  **image tokens cost 0.47 on that model, ~4× text**, which our one-price-per-model table cannot
  ...
  anything embeds an image (§4.9; Embed 4 is multimodal with a 128K window, so diagram retrieval for
  figure-heavy chapters makes that plausible). **At D23** — re-run the milestone economics against
