...
HOW TO RESUME — the spike is done; what it leaves open, in order:
  1. **Widen the answer key on q06 and q08** (DECISIONS): ch 4 §4.10 ¶18 and ch 6 §6.9 ¶2 are
     better answers than the addresses those two queries name, and the headline number must not be
     quoted as D17's baseline until the key is right. ₹0 — a re-run of `ncert embed` re-scores for
     the price of 15 query embeddings, since every paragraph is already embedded.
  ...
  3. **Before D17, get a production embedding key.** The trial key's 100 calls/minute is why the
     book took ten minutes rather than two; the cap is on rate, not spend, so a production key
     costs the same per token and lets `embed-calls-per-minute` go to 0. Ten books is ~90 minutes
     of pacing today, and D17 re-embeds the whole corpus.
