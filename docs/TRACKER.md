...
THE KEY IS CORRECTED, founder's instruction, ₹0 — by reading §4.10 and §6.9 cold rather than by
  ...
  The set now reads **hit@1 8/15 (53%), hit@3 10/15 (67%), MRR 0.618** — english 6/10, 7/10, 0.677;
  hindi unchanged at 2/5, 3/5, 0.500. Computed from the run's own top-3 listing, and the same
  arithmetic reproduces the reported 7/15 · 9/15 · 0.576 exactly, so the projection is sound; the
  authoritative number is a re-run of `ncert embed`, which re-scores for 15 query embeddings.
...
HOW TO RESUME — the spike is done; what it leaves open, in order:
  ...
  2. **Before D17, get a production embedding key.** The trial key's 100 calls/minute is why the
