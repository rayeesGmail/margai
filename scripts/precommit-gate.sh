...
#   3. AI-touching paths (server prompts/ and ai/, routing or retrieval paths under
#      server/ or eval/, eval fixtures) require eval/run.sh to have PASSED on exactly
#      this content (eval/.last-pass stamp). The app's go_router lives in app/lib/core/router/
#      and is not the AI difficulty router, so app/ is outside the router alternative (D12).
...
AI_PATHS='^server/src/main/resources/prompts/|^server/.*/ai/|^(server|eval)/([^/]*/)*[^/]*(router|routing|retriev)[^/]*(/|$)|^eval/fixtures/.+\.(json|jsonl|ya?ml|csv)$'
