 ...
 * client address for {@code POST /auth/otp/request} (10/hour) and by user id for every
 * authenticated request (60/min); values from {@code margai.limits.*}. An exhausted bucket answers
 * 429 with {@code Retry-After} and the envelope, without reaching the controller. The
 * per-destination OTP cap is not here — it is a durable check in {@code OtpService}. Buckets refill
 * greedily and idle ones are evicted after an hour; this holds for one API task (§13.3).
 */
final class RateLimitFilter extends OncePerRequestFilter {
    ...
    private Rule ruleFor(HttpServletRequest request) {
        if ("POST".equals(request.getMethod()) && OTP_REQUEST_PATH.equals(request.getRequestURI())) {
