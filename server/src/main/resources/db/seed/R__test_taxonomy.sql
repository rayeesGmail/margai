...
-- idempotent — INSERT ... ON CONFLICT keyed on the row's fixed id, so any value including a
-- natural key (code, sequence, cutoff key) can be edited and re-applied on a persistent local
-- db; only prerequisite edges, whose primary key is the edge itself, use DO NOTHING. Rows carry
-- fixed UUIDs of the form 00000000-0000-4000-8000-0000000000NN so tests can reference them:
--   01–02 subjects, 11–12 units, 21–26 chapters, 31–32 topics, 41 track, 51–57 steps, 61–63 cutoffs.
--
-- No rollback block: seed data is not schema, and the V3/V4 table drops remove it. To clear only
-- the seed on a local db, delete the rows above by id in reverse dependency order.
